 ...
 *
 * No schedule matching: real services run with long, unpredictable delays, so the clock
 * says nothing reliable about which leg we're on — only the GPS does. Stateful but
 * otherwise pure, so the logic is unit-tested directly.
 */
class LegAdvancer(
    ...
    /**
     * Feed one fix. [tMs] is the fix's timestamp in millis (dwell timing). Returns true
     * exactly when to advance to the next leg.
     */
    fun onFix(lat: Double, lon: Double, tMs: Long): Boolean {
